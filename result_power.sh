	    ...
	    for check_rate in "${check_rates[@]}"; do
	      for sigma in "${sigmas[@]}"; do
					for density in "${densities[@]}"; do
						...
						for seed in "${seeds[@]}"; do
							if [ -f $result_dir/results/$filename-$seed ]; then
								cat $result_dir/results/$filename-$seed >> temp
								rm -f $result_dir/results/$filename-$seed
							fi
						done 
	  ...
	  for sigma in "${sigmas[@]}"; do
	    for density in "${densities[@]}"; do
				for topology in "${topologies[@]}"; do
					...
					if [ -f ../$result_dir/results/$period-$topology-$min_rate-$density ]; then
